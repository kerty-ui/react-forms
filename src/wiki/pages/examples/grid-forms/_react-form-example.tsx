...
const ReactFormExample = () => {
    const [data, setData] = useState<GridForm>({
        rows: [],
    });
    ...
    const appendRows = useCallback((count: number) => {
        setData(prevState => {
            return {
                ...
                rows: [
                    ...prevState.rows,
                    ...Array.from({ length: count }, () => getDefaultRowData())
                ]
            }
        });
    }, []);
    ...
    const setRowValue = useCallback((index: number, name: string, value: number | null) => {
        setData(prevState => {
            return {
                ...prevState,
                rows: prevState.rows.map((row, i) => {
                    return i === index ? {...row, [name]: value } : row;
                })
            };
        });
    }, []);
        ...
        <article>
            ...
            <div>
                ...
                <div>
                    ...
                    <button type="button" onClick={() => setData({ rows: []})}>
                        Clear
                    </button>
                </div>
            </div>
...
const Row = memo((props: {
    ...
    setRowValue: (index: number, name: string, value: number | null) => void;
}) => {
    return (
        <tr>
            <td>{props.index + 1}</td>
...
export const ReactFormExample = () => {
    ...
    return (
        <ExampleBlock
            title="Editable Grid with useState"
            description={
                <>
                    <p>Demonstrates an editable data grid built with React's <b>useState</b>. Each row is wrapped in <b>memo</b> and update callbacks are stabilized with <b>useCallback</b> to avoid unnecessary re-renders.</p>
